...
PostgreSQL 17 локально, 18 в CI и на Neon. Схема описана миграциями [dbmate](https://github.com/amacneil/dbmate) на чистом SQL в `db/migrations/`. Приложение работает с базой через драйвер `pg` без ORM. Авторизация — [Better Auth](https://www.better-auth.com/) (`lib/auth.ts`), его таблицы тоже создаются миграциями.
...
- Если `pg_dump` и `pg_restore` нет в `PATH`, скрипты запускают их в Docker-образе `postgres:18-alpine` (pg_dump должен быть не старше сервера; продакшен на Neon — PostgreSQL 18). Другой образ задается переменной `PG_IMAGE`.
